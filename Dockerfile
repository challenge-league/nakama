...
RUN packr clean && \
    packr -z && \
    go build -o /go/build-out/nakama -trimpath -gcflags "-trimpath $PWD" -asmflags "-trimpath $PWD" -ldflags "-s -w"
...
WORKDIR /nakama/
COPY --from=localhost:32000/nakama-plugin:dkozlov /go/build/nakama-plugin /nakama/data/modules/challenge-league
COPY --from=builder /go/build-out/nakama /nakama/
COPY dataleague-org-2846b61502d2.json .
